  ...
  for (const q of queries) {
    try {
      ...
      try {
        ...
        // Network error — don't cache, just skip this query
        continue;
      }

      // API error (bad key, quota exceeded) — throw so caller knows not to cache
      if (data?.error) {
        const code = data.error.code;
        if (code === 400 || code === 401 || code === 403) {
          throw new Error(`Google Books API error ${code}: ${data.error.message}`);
        }
        if (code === 429) {
          await sleep(10000);
          throw new Error("Google Books rate limit 429");
        }
        continue; // other errors — skip query
      }
...
async function processBook(
  ...
  try {
    ...
    const isRateLimit = msg === "RATE_LIMIT" || msg.includes("429") || msg.includes("rate limit");
    const isApiKeyError = msg.includes("API error 400") || msg.includes("API error 403") || msg.includes("API_KEY_INVALID");
    // Rate limit or key error: always keep pending, don't count attempt
    state.set(input.id, {
      ...(existing ?? emptyRecord(input, msg)),
      status: "pending",
      attempt_count: (isRateLimit || isApiKeyError) ? (existing?.attempt_count ?? 0) : attempt,
      processed_at: new Date().toISOString(),
      error_log: msg,
    });
    return "failed";
...
async function main() {
  console.log("\n📚 Book Enrichment — Gemini Vision v10\n");
