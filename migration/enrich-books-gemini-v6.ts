...
function resolveImageUrl(book: InputBook): { url: string; fileId: string | null } | null {
  ...
  if (fileId) {
    // Use direct Drive download URL — most reliable for server-side download
    return { url: `https://drive.google.com/uc?export=download&id=${fileId}&confirm=t`, fileId };
  }
...
async function extractWithGemini(imageUrl: string): Promise<RawFields> {
  // Strategy: download image ourselves, send as base64.
  // We try 4 different URL formats for Drive images, in order.
  const fileId = imageUrl.match(/[?&]id=([a-zA-Z0-9_-]{10,})/)?.[1]
  ...
  // Try to download a real image from any of the candidate URLs
  let imgBase64 = "";
    ...
    try {
      const buf = await new Promise<Buffer>((resolve, reject) => {
        function doGet(u: string, hops = 0): void {
          if (hops > 8) { reject(new Error("redirect loop")); return; }
          const pu = new URL(u);
          https.get({
            ...
            res.on("end", () => {
              ...
              // Must be an actual image (not HTML error page)
              const firstBytes = b.slice(0, 20).toString("ascii");
              const isHtml = b[0] === 0x3c
  ...
  // If all downloads failed — tell Gemini to describe what it can from the URL
  // (last-resort: send just the original URL as text context)
  if (!imgBase64) {
    throw new Error(`Could not download image from any URL (fileId=${fileId ?? "none"})`);
  }

  if (process.env.DEBUG_SCORE) {
    process.stdout.write(` [img:${Math.round(imgBase64.length * 0.75 / 1024)}kb via ${usedUrl.slice(0, 50)}]`);
  }
  ...
  const responseText = await withRetry(() => httpPost(
...
function strSim(a: string, b: string): number {
  ...
  // Containment — handles partial titles / subtitle cut-off
  // e.g. Vision reads "The Religions of India: Handbooks on"
  //      Google Books has "The Religions of India"
  if (nb.length >= 6 && na.includes(nb)) return 0.95;
  if (na.length >= 6 && nb.includes(na)) return 0.95;

  // Prefix match — 60% of shorter string
  const minLen = Math.min(na.length, nb.length);
  const prefixLen = Math.floor(minLen * 0.6);
  if (prefixLen >= 8 && na.slice(0, prefixLen) === nb.slice(0, prefixLen)) return 0.88;

  // Compare main titles only (strip subtitle after : or dash)
  const mainA = na.split(/[:\u2014\u2013]|--/)[0].trim();
  const mainB = nb.split(/[:\u2014\u2013]|--/)[0].trim();
  if (mainA.length >= 5 && mainB.length >= 5) {
    if (mainA === mainB) return 0.92;
    if (mainB.includes(mainA) || mainA.includes(mainB)) return 0.88;
    const ms = trigramSim(mainA, mainB);
    if (ms > 0.7) return 0.75 + ms * 0.15;
  }

  // Significant word overlap
  const wordsA = na.split(" ").filter((w: string) => w.length > 3);
  const wordsB = nb.split(" ").filter((w: string) => w.length > 3);
  if (wordsA.length >= 2 && wordsB.length >= 2) {
    const common = wordsA.filter((w: string) => wordsB.includes(w)).length;
    const ws = common / Math.max(wordsA.length, wordsB.length);
    if (ws >= 0.5) return 0.60 + ws * 0.25;
  }

  return trigramSim(na, nb);
}
...
async function main() {
  console.log("\n📚  Book Enrichment — Gemini Vision v6\n");
