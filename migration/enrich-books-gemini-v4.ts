...
function resolveImageUrl(book: InputBook): { url: string; fileId: string | null } | null {
  ...
  return { url: `https://drive.google.com/thumbnail?id=${fileId}&sz=w1000`, fileId };
  }
...
async function extractWithGemini(imageUrl: string): Promise<RawFields> {
  // Download image with Drive-compatible headers
  const imgBuf = await withRetry(() => new Promise<Buffer>((resolve, reject) => {
    function doGet(url: string, redirects = 0): void {
      ...
      https.get({
        ...
        res.on("end", () => {
          ...
          if (buf[0] === 0x3c || buf.length < 500) {
            reject(new Error(`HTML response or empty image (${buf.length}b) from ${url.slice(0, 60)}`));
            return;
          }
          resolve(buf);
        });
  ...
  if (imgBuf[0] === 0x89 && imgBuf[1] === 0x50) mimeType = "image/png";
...
if (process.env.DEBUG_SCORE) {
  process.stdout.write(
    `\n[DEBUG] raw_title="${raw.raw_title}" | raw_author="${raw.raw_author}" | gb_title="${gb?.google_books_title}" | gb_author="${gb?.google_books_author}" | score=${candidates[0]?.score}\n`
  );
}
...
async function main() {
  console.log("\n📚  Book Enrichment — Gemini Vision v4\n");
